  ...
  getPosition(data, posMouse) {
    ...
    switch (this.config.magnet.position) {
      ...
      case "top-right":
        x =
          posMouse.x -
          (data.xMin + data.elem.width + this.config.magnet.distance);
        y = posMouse.y - (data.yMin + this.config.magnet.distance);
        break;
      ...
      case "bottom-left":
        ...
        y =
          posMouse.y -
          (data.yMin + data.elem.height + this.config.magnet.distance);
        break;
      ...
      case "top-center":
        x =
          posMouse.x -
          (data.xMin + this.config.magnet.distance + data.elem.width / 2);
        y = posMouse.y - (data.yMin + this.config.magnet.distance);
        break;
      ...
      default:
        ...
        y =
          posMouse.y -
          (data.yMin + this.config.magnet.distance + data.elem.height / 2);
    }
  ...
  defaultAction(action, mouseElement, data) {
    if (action === "onEnter") {
      ...
      data.elem.node.style.transform =
        "translate3d(" + mouseElement.x + "px," + mouseElement.y + "px, 0)";
  ...
  magnetElement(posElement, posMouse) {
    if (posElement) {
      for (let data of posElement) {
        ...
        if (
          data.xMin < posMouse.x &&
          data.xMax > posMouse.x &&
          data.yMin < posMouse.y &&
          data.yMax > posMouse.y
        ) {
          this.defaultAction("onEnter", mouseElement, data);

          // Callback when mouse enter in element else add class
          if (
            this.config.inCallback !== null &&
            typeof this.config.inCallback === "function"
          ) {
          ...
          if (
            this.config.outCallback !== null &&
            typeof this.config.outCallback === "function"
          ) {
        ...
        if (this.elementFollow.length > 0) {
          this.elementFollow.forEach((element) => {
            element.style.transform =
              "translate3d(" +
              (posMouse.x - window.pageXOffset) +
              "px," +
              (posMouse.y - window.pageYOffset) +
              "px, 0)";
          });
        }
      }, this.config.throttle);
